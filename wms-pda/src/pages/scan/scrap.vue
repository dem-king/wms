<!--
  WMS-PDA 报废扫码页面
  扫码后校验labelStatus=1(在库)或5(闲置)才可报废
-->
<template>
  <view class="scan-scrap-page">
    <!-- 库房选择 -->
    <view class="scan-scrap-page__field">
      <text class="scan-scrap-page__label">选择库房</text>
      <picker
        class="scan-scrap-page__picker"
        :range="warehouses"
        range-key="warehouseName"
        @change="handleWarehouseChange"
      >
        <view class="scan-scrap-page__picker-value">
          <text v-if="selectedWarehouse">{{ selectedWarehouse.warehouseName }}</text>
          <text v-else class="scan-scrap-page__placeholder">请选择库房</text>
        </view>
      </picker>
    </view>

    <!-- 扫码输入 -->
    <view class="scan-scrap-page__scan">
      <ScanInput
    ...
    <!-- 扫码明细列表 -->
    <view class="scan-scrap-page__details">
      <view class="scan-scrap-page__details-header">
        <text class="scan-scrap-page__details-title">扫码明细</text>
        <text class="scan-scrap-page__details-count">共 {{ details.length }} 条</text>
      </view>

      <view v-if="details.length > 0" class="scan-scrap-page__details-list">
        <DetailItem
    ...
    <!-- 底部操作栏 -->
    <view class="scan-scrap-page__footer">
      <view
        class="scan-scrap-page__btn scan-scrap-page__btn--clear"
        @click="handleClear"
        v-if="details.length > 0"
      >
        <text class="scan-scrap-page__btn-text">清空</text>
      </view>
      <view
        class="scan-scrap-page__btn scan-scrap-page__btn--submit"
        :class="{ 'scan-scrap-page__btn--disabled': !canSubmit }"
        @click="handleSubmit"
      >
        <text class="scan-scrap-page__btn-text scan-scrap-page__btn-text--submit">
          {{ scanning ? '扫码中...' : '提交' }}
        </text>
      </view>
    </view>
...
/**
 * 报废扫码页面
 * 选库房 → 扫码 → 校验labelStatus=1(在库)或5(闲置) → 添加明细 → 删除 → 提交
 */
