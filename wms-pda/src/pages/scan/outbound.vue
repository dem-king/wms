<!--
  WMS-PDA 出库扫码页面
  与入库扫码流程对称，调用出库扫码识别接口
-->
<template>
  <view class="scan-outbound-page">
    <!-- 库房选择 -->
    <view class="scan-outbound-page__field">
      <text class="scan-outbound-page__label">选择库房</text>
      <picker
        class="scan-outbound-page__picker"
        :range="warehouses"
        range-key="warehouseName"
        @change="handleWarehouseChange"
      >
        <view class="scan-outbound-page__picker-value">
          <text v-if="selectedWarehouse">{{ selectedWarehouse.warehouseName }}</text>
          <text v-else class="scan-outbound-page__placeholder">请选择库房</text>
        </view>
      </picker>
    </view>

    <!-- 扫码输入 -->
    <view class="scan-outbound-page__scan">
      <ScanInput
    ...
    <!-- 扫码明细列表 -->
    <view class="scan-outbound-page__details">
      <view class="scan-outbound-page__details-header">
        <text class="scan-outbound-page__details-title">扫码明细</text>
        <text class="scan-outbound-page__details-count">共 {{ details.length }} 条</text>
      </view>

      <view v-if="details.length > 0" class="scan-outbound-page__details-list">
        <DetailItem
    ...
    <!-- 底部操作栏 -->
    <view class="scan-outbound-page__footer">
      <view
        class="scan-outbound-page__btn scan-outbound-page__btn--clear"
        @click="handleClear"
        v-if="details.length > 0"
      >
        <text class="scan-outbound-page__btn-text">清空</text>
      </view>
      <view
        class="scan-outbound-page__btn scan-outbound-page__btn--submit"
        :class="{ 'scan-outbound-page__btn--disabled': !canSubmit }"
        @click="handleSubmit"
      >
        <text class="scan-outbound-page__btn-text scan-outbound-page__btn-text--submit">
          {{ scanning ? '扫码中...' : '提交' }}
        </text>
      </view>
    </view>
