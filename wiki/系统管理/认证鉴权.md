...
| 登录保护 | `RateLimiterService` / `CaptchaService` / `LoginLockService` | IP 限流、滑块拼图验证码（tianai-captcha + IP/UA 指纹绑定）、失败计数与账号锁定 |
...
sequenceDiagram
    ...
    FE->>Ctrl: POST /auth/login {username, encryptedPassword, captchaToken, captchaTrack}
    ...
    alt 启用了验证码
        Svc->>Svc: 校验 captchaToken/Track 非空 (缺失 → CAPTCHA_REQUIRED)
        Svc->>Cap: validateCaptcha(token, track, ip, ua)
        Cap->>Redis: GETDEL auth:captcha:fp:{token}   (一次性消费客户端指纹)
        alt 指纹缺失或不匹配
            Cap-->>Svc: 抛 CAPTCHA_INVALID
        else 指纹通过
            Cap->>Cap: objectMapper 解析 track JSON 为 ImageCaptchaTrack
            Cap->>Tianai: imageCaptchaApplication.matching(token, track)
            Tianai-->>Cap: success=true / false
            alt success=false
                Cap-->>Svc: 抛 CAPTCHA_MISMATCH
            end
        end
    end
...
| 2. 验证码 | `authProperties.isCaptchaEnabled()` 为 `true` 时**强制**要求前端传 `captchaToken/Track`，缺一即抛 `CAPTCHA_REQUIRED`（不允许"前端不传 captcha 字段即可跳过校验"的绕过后门）；校验含三层：①Redis 指纹 getAndDelete ②IP+UA 哈希绑定 ③tianai SDK 轨迹 matching | `CAPTCHA_REQUIRED` / `CAPTCHA_INVALID` / `CAPTCHA_MISMATCH` |
...
> 2026-06-03 由 Hutool `LineCaptcha` 图形码切换为 **tianai-captcha-springboot-starter 1.5.2** 滑块拼图；接口从 `/auth/captcha/image` 改为 `/auth/captcha/slider`，请求/响应字段、Redis Key、错误码、审计均同步调整。详见 [`docs/plans/2026-06-03-slider-puzzle-captcha.md`](../../docs/plans/2026-06-03-slider-puzzle-captcha.md)。
...
stringRedisTemplate.opsForValue().set(
        "auth:captcha:fp:" + token,            // 项目自管指纹 Key，与 tianai 内部缓存解耦
        fingerprint,
        Duration.ofSeconds(authProperties.getCaptchaExpire()));   // 默认 120s
...
resp.setBlockY(((SliderImageCaptchaInfo) vo.getData()).getY());
resp.setExpiresIn((int) authProperties.getCaptchaExpire());
return resp;
```

返回 `CaptchaImageResp { captchaToken, backgroundImage(Base64), blockImage(Base64), blockY, expiresIn }`。

### 6.2 校验（三层）

`CaptchaServiceImpl#validateCaptcha(token, trackJson, clientIp, userAgent)`：

```java
// 第 1 层: 客户端指纹一次性消费（getAndDelete）
String storedFingerprint = stringRedisTemplate.opsForValue()
        .getAndDelete("auth:captcha:fp:" + token);
if (storedFingerprint == null) throw new BizException(CAPTCHA_INVALID);

// 第 2 层: 当前请求 IP+UA 哈希必须与生成时一致（防 token 被其他客户端截获重放）
String currentFingerprint = DigestUtil.sha256Hex(clientIp + "|" + userAgentParser.truncate(userAgent));
if (!currentFingerprint.equals(storedFingerprint)) throw new BizException(CAPTCHA_INVALID);

// 第 3 层: 解析前端 track JSON → tianai SDK matching（容差由 captchaTrackTolerance 控制）
ImageCaptchaTrack track = objectMapper.readValue(captchaTrack, ImageCaptchaTrack.class);
ApiResponse<?> response = imageCaptchaApplication.matching(token, track);
if (!response.isSuccess()) throw new BizException(CAPTCHA_MISMATCH);
```

| 层级 | 作用 | 失败错误码 |
|------|------|----------|
| 1. 指纹 getAndDelete | 一次性消费（与 tianai 内部 captcha 缓存解耦） | `CAPTCHA_INVALID` |
| 2. IP+UA 哈希绑定 | 防止 token 被其他 IP/UA 截获重放 | `CAPTCHA_INVALID` |
| 3. tianai `matching` | 坐标/轨迹/时间窗采样校验（容差 ±`captchaTrackTolerance` px） | `CAPTCHA_MISMATCH` |

> **共享 code=400 的区分**：`CAPTCHA_INVALID`（指纹层）与 `CAPTCHA_MISMATCH`（轨迹层）共用 HTTP 400，但 msg 不同（"验证码无效或已过期" vs "验证未通过，请重试"）。`AuthServiceImpl#login` 通过 `AuthErrorCode.CAPTCHA_INVALID.getMsg().equals(e.getMessage())` 按 msg 区分审计原因。

### 6.3 启用开关与强制要求

`authProperties.isCaptchaEnabled()` 默认 `true`。`AuthServiceImpl#login` 在开关**为 true** 时：

1. **强制**要求 `captchaToken` / `captchaTrack` 两者**均非空**（`StringUtils.hasText` 判断）；缺一抛 `CAPTCHA_REQUIRED` 并写审计。
2. 缺一即失败，**不允许**"前端不传 captcha 字段跳过校验"的绕过后门（旧实现的漏洞）。
3. 开关为 `false` 时，登录主流程完全跳过验证码（仅供联调/压测环境临时关闭）。

```java
if (authProperties.isCaptchaEnabled()) {
    if (!StringUtils.hasText(req.getCaptchaToken()) || !StringUtils.hasText(req.getCaptchaTrack())) {
        authAuditService.recordCaptchaFailure("CAPTCHA_REQUIRED", clientIp, userAgent, req.getCaptchaToken());
        throw new BizException(AuthErrorCode.CAPTCHA_REQUIRED);
    }
    try {
        captchaService.validateCaptcha(req.getCaptchaToken(), req.getCaptchaTrack(), clientIp, userAgent);
    } catch (BizException e) {
        String reason = AuthErrorCode.CAPTCHA_INVALID.getMsg().equals(e.getMessage())
                ? "CAPTCHA_INVALID" : "CAPTCHA_MISMATCH";
        authAuditService.recordCaptchaFailure(reason, clientIp, userAgent, req.getCaptchaToken());
        throw e;
    }
}
```
...
`CaptchaController#getSliderCaptcha` 入口处通过 `RateLimiterService` 复用 IP 限流（与登录共用 Lua 脚本），阈值由 `authProperties.getCaptchaRateLimitThreshold()` 控制（默认 **20 次/60 秒**）。超额抛 `CAPTCHA_FREQUENT`（HTTP 429）"验证码请求过于频繁"。

> 该限流**仅针对** `/auth/captcha/slider` 接口；登录接口的 IP 限流仍由 `ipRateLimitThreshold`（默认 10/60s）控制。

### 6.5 滑块坐标容差

`authProperties.getCaptchaTrackTolerance()` 控制 tianai `matching` 的容差（默认 **5 px**），即拼图块中心 X 坐标落在目标位 ±5px 范围内视为通过。该值过松会降低安全性，过严会被鼠标采样误差影响通过率。

### 6.6 失败审计

`AuthAuditServiceImpl#recordCaptchaFailure(reason, ip, ua, token)` 在以下三种场景被调用：

| 触发点 | reason | 写入字段 |
|--------|--------|---------|
| `AuthServiceImpl#login` 入口 | `CAPTCHA_REQUIRED` | `username=__captcha__` 占位 |
| 指纹层失败 | `CAPTCHA_INVALID` | `fail_reason = "captcha:CAPTCHA_INVALID:token=tok-1234"` |
| 轨迹层失败 | `CAPTCHA_MISMATCH` | `fail_reason = "captcha:CAPTCHA_MISMATCH:token=tok-1234"` |

- `username` 固定写 `__captcha__`（`AuthAuditServiceImpl.CAPTCHA_AUDIT_USERNAME`），用于在登录日志中按"未通过验证码校验的事件"检索。
- `token` 仅截取前 8 位写入日志（`CAPTCHA_TOKEN_LOG_PREFIX=8`），避免完整 Token 落入数据库。
- `@Async` 异步落库 `sys_login_log`，异常仅 `log.warn`，不影响登录主流程。

### 6.7 短信验证码

当前实现**仅滑块拼图**。如接入短信验证码，建议：
...
| `CAPTCHA_PREFIX` | `auth:captcha:` | 滑块拼图客户端指纹（仅 `auth:captcha:fp:{token}` 一支；tianai 内部缓存走另一套独立 Key） | captchaExpire（120s） |
...
| `CaptchaServiceImpl#validateCaptcha`（指纹缺失/不匹配） | `CAPTCHA_INVALID`（400） | "验证码无效或已过期" | Redis `getAndDelete` 返回 null 或 SHA-256(IP\|UA) 与生成时不匹配 |
| `CaptchaServiceImpl#validateCaptcha`（轨迹错误） | `CAPTCHA_MISMATCH`（400） | "验证未通过，请重试" | tianai `matching` 返回 success=false（含坐标超出 `captchaTrackTolerance`） |
| `AuthServiceImpl#login`（缺 captchaToken/Track） | `CAPTCHA_REQUIRED`（400） | "请完成滑块验证" | 开关开启时强制要求，缺一即失败 |
| `CaptchaController#getSliderCaptcha`（IP 限流） | `CAPTCHA_FREQUENT`（429） | "验证码请求过于频繁" | 60s 窗口内超过 `captchaRateLimitThreshold`（默认 20） |
...
| `wms.auth.captcha-expire` | `120` 秒（2min） | 可热更新（`wms.security.captcha-expire-seconds`） |
| `wms.auth.captcha-rate-limit-threshold` | `20` | 可热更新（与登录共用 IP 限流 Lua，60s 窗口） |
| `wms.auth.captcha-track-tolerance` | `5` px | tianai `matching` 容差，过松降低安全性 |
...
wms:
  auth:
    ...
    captcha-expire: 120
    captcha-rate-limit-threshold: 20
    captcha-track-tolerance: 5
...
10. **`CaptchaService` 滑块拼图指纹 `getAndDelete` 是原子的**：指纹一次消费即删除；前端若失败重试会**永远报 `CAPTCHA_INVALID`**，需提示用户重新 `/auth/captcha/slider` 拉新拼图（同时建议前端在 `CAPTCHA_MISMATCH` 时刷新拼图、`CAPTCHA_INVALID` 时强制刷新）。
