...
| wms-approval | wms-server/wms-approval/ | 审批流程（预留） |
| wms-report | wms-server/wms-report/ | 报表模块（预留） |
...
### 1. 需求理解与澄清
- 收到需求后，先加载 `project-analysis` skill 理解项目现状
- 对模糊需求主动提问澄清
- 产出 spec.md（需求分析文档）
...
- **触发**: 需求分析确认
- **加载**: `coding-skill`
- **产出**: 代码变更 + `changes/{变更名}/coding_report.md`
