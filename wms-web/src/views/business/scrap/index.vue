<template>
  <div class="app-container list-page">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button v-if="userStore.hasPermission('business:scrap:add')" type="primary" plain :icon="Plus" @click="handleAdd">新增</el-button>
      </el-col>
    </el-row>
        ...
        <template #default="{ row }">
          <TableActionGroup
            :actions="[
              ...
              { label: '编辑', type: 'primary', icon: Edit, permission: 'business:scrap:edit', visible: isDraftOrderStatus(row.status), onClick: () => handleEdit(row) },
              { label: '提交', type: 'warning', permission: 'business:scrap:submit', visible: isDraftOrderStatus(row.status), onClick: () => handleSubmitOrder(row) },
              { label: '删除', type: 'danger', icon: Delete, permission: 'business:scrap:delete', visible: isDraftOrderStatus(row.status), confirmText: '确定删除该报废单吗？', onClick: () => handleDelete(row.id) },
            ]"
          />
        </template>
      </el-table-column>
...
import type { ScrapOrderVo } from '@/types/business'
