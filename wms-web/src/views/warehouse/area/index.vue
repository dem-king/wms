<template>
  <div class="app-container list-page">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button v-if="userStore.hasPermission('warehouse:area:add')" type="primary" plain :icon="Plus" :disabled="!selectedWarehouseId" @click="handleAdd">新增</el-button>
      </el-col>
    </el-row>
      ...
      <el-table v-loading="loading" :data="tableData" border height="100%">
        ...
        <el-table-column label="操作" class-name="table-action-column" fixed="right" min-width="180">
          <template #default="{ row }">
            <TableActionGroup
              :actions="[
                { label: '编辑', type: 'primary', icon: Edit, permission: 'warehouse:area:edit', onClick: () => handleEdit(row) },
                { label: '删除', type: 'danger', icon: Delete, permission: 'warehouse:area:delete', confirmText: '确定删除该区域吗？', onClick: () => handleDelete(row.id) },
              ]"
            />
          </template>
        </el-table-column>
...
onMounted(async () => {
  ...
  if (res.data.length > 0) {
    ...
    await handleQuery(res.data[0].id)
  }
})
</script>
