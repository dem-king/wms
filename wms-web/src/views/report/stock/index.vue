...
import { getStockSummary, getStockTrend, getStockCategoryDistribution, exportReportExcel, exportReportPdf } from '@/api/report'
import { downloadBlob, getExportFilename } from '@/utils/export'
import type { StockSummaryVo, StockTrendVo, StockDistributionVo } from '@/types/report'

const loading = ref(false)
const summary = ref<StockSummaryVo | null>(null)
const trendData = ref<StockTrendVo | null>(null)
const distributionData = ref<StockDistributionVo | null>(null)
...
async function handleQuery() {
  ...
  try {
    ...
    const [summaryRes, trendRes, distRes] = await Promise.all([
      ...
      getStockCategoryDistribution(params)
    ])
...
function buildTrendChart(data: StockTrendVo) {
...
function buildDistributionChart(data: StockDistributionVo) {
...
<template>
  <div class="app-container">
    ...
    <el-row :gutter="20" class="summary-cards" v-loading="loading">
      <el-col :span="12"><el-card shadow="hover"><div class="card-value">{{ summary?.totalQuantity ?? '-' }}</div><div class="card-label">库存总数量</div></el-card></el-col>
      <el-col :span="12"><el-card shadow="hover"><div class="card-value">{{ summary?.totalAmount ?? '-' }}</div><div class="card-label">库存总金额</div></el-card></el-col>
    </el-row>
