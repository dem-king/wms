...
import { getInboundSummary, getInboundTrend, getInboundCategoryDistribution, exportReportExcel, exportReportPdf } from '@/api/report'
import { downloadBlob, getExportFilename } from '@/utils/export'
import type { InboundSummaryVo, InboundTrendVo, InboundDistributionVo } from '@/types/report'

const loading = ref(false)
const summary = ref<InboundSummaryVo | null>(null)
const trendData = ref<InboundTrendVo | null>(null)
const distributionData = ref<InboundDistributionVo | null>(null)
...
async function handleQuery() {
  ...
  try {
    ...
    const [summaryRes, trendRes, distRes] = await Promise.all([
      getInboundSummary(params),
      getInboundTrend({ ...params, trendType: 'DAILY' }),
      getInboundCategoryDistribution(params)
    ])
...
function buildTrendChart(data: InboundTrendVo) {
...
function buildDistributionChart(data: InboundDistributionVo) {
  const pieData = data.distributionList.map(item => ({
    name: item.categoryName,
    value: item.quantity
  }))
  distributionChartOption.value = {
    title: { text: '入库分类分布', left: 'center' },
...
async function handleExport(exportType: string) {
  ...
  try {
    const params = buildParams()
    const data = { ...params, reportType: 'inbound', exportType }
    const res = exportType === 'EXCEL'
      ? await exportReportExcel(data)
      : await exportReportPdf(data)
    const ext = exportType === 'EXCEL' ? 'xlsx' : 'pdf'
    downloadBlob(res as unknown as Blob, getExportFilename('inbound', ext))
...
<template>
  <div class="app-container">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="success" plain :icon="Download" :loading="exporting" @click="handleExport('EXCEL')">导出Excel</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain :icon="Download" :loading="exporting" @click="handleExport('PDF')">导出PDF</el-button>
      </el-col>
    </el-row>
    ...
    <el-row :gutter="20" class="summary-cards" v-loading="loading">
      <el-col :span="8">
        <el-card shadow="hover">
          ...
          <div class="card-label">入库总数量</div>
        </el-card>
      </el-col>
      <el-col :span="8">
        <el-card shadow="hover">
          <div class="card-value">{{ summary?.totalAmount ?? '-' }}</div>
          <div class="card-label">入库总金额</div>
        </el-card>
      </el-col>
      <el-col :span="8">
        <el-card shadow="hover">
          <div class="card-value">{{ summary?.orderCount ?? '-' }}</div>
          <div class="card-label">入库单据数</div>
        </el-card>
      </el-col>
    </el-row>
    ...
    <el-row :gutter="20" style="margin-top: 20px">
      ...
      <el-col :span="10">
        <el-card shadow="never">
          <Chart :option="distributionChartOption" height="350px" />
        </el-card>
      </el-col>
    </el-row>
