...
import { getReturnSummary, getReturnTrend, getReturnCategoryDistribution, exportReportExcel, exportReportPdf } from '@/api/report'
import { downloadBlob, getExportFilename } from '@/utils/export'
import type { ReturnSummaryVo, ReturnTrendVo, ReturnDistributionVo } from '@/types/report'

const loading = ref(false)
const summary = ref<ReturnSummaryVo | null>(null)
const trendData = ref<ReturnTrendVo | null>(null)
const distributionData = ref<ReturnDistributionVo | null>(null)
...
async function handleQuery() {
  ...
  try {
    ...
    const [summaryRes, trendRes, distRes] = await Promise.all([
      ...
      getReturnTrend({ ...params, trendType: 'DAILY' }),
      getReturnCategoryDistribution(params)
    ])
...
function buildTrendChart(data: ReturnTrendVo) {
...
<template>
  <div class="app-container">
    ...
    <el-row :gutter="20" class="summary-cards" v-loading="loading">
      <el-col :span="6"><el-card shadow="hover"><div class="card-value">{{ summary?.totalQuantity ?? '-' }}</div><div class="card-label">归还总数量</div></el-card></el-col>
      <el-col :span="6"><el-card shadow="hover"><div class="card-value">{{ summary?.normalQuantity ?? '-' }}</div><div class="card-label">正常归还数量</div></el-card></el-col>
      <el-col :span="6"><el-card shadow="hover"><div class="card-value">{{ summary?.damagedQuantity ?? '-' }}</div><div class="card-label">损坏归还数量</div></el-card></el-col>
      <el-col :span="6"><el-card shadow="hover"><div class="card-value">{{ summary?.normalRate != null ? summary.normalRate + '%' : '-' }}</div><div class="card-label">正常归还率</div></el-card></el-col>
    </el-row>
    ...
    <el-card shadow="never" style="margin-top: 20px" v-if="summary?.categorySummaryList?.length">
      <template #header><span>分类汇总明细</span></template>
