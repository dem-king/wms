...
import { getScrapSummary, getScrapTrend, getScrapCategoryDistribution, exportReportExcel, exportReportPdf } from '@/api/report'
import { downloadBlob, getExportFilename } from '@/utils/export'
import type { ScrapSummaryVo, ScrapTrendVo, ScrapDistributionVo } from '@/types/report'

const loading = ref(false)
const summary = ref<ScrapSummaryVo | null>(null)
const trendData = ref<ScrapTrendVo | null>(null)
const distributionData = ref<ScrapDistributionVo | null>(null)
...
async function handleQuery() {
  ...
  try {
    ...
    const [summaryRes, trendRes, distRes] = await Promise.all([
      ...
      getScrapTrend({ ...params, trendType: 'DAILY' }),
      getScrapCategoryDistribution(params)
    ])
...
function buildTrendChart(data: ScrapTrendVo) {
...
<template>
  <div class="app-container">
    ...
    <el-row :gutter="20" class="summary-cards" v-loading="loading">
      <el-col :span="8"><el-card shadow="hover"><div class="card-value">{{ summary?.totalQuantity ?? '-' }}</div><div class="card-label">报废总数量</div></el-card></el-col>
      <el-col :span="8"><el-card shadow="hover"><div class="card-value">{{ summary?.totalAmount ?? '-' }}</div><div class="card-label">报废总金额</div></el-card></el-col>
      <el-col :span="8"><el-card shadow="hover"><div class="card-value">{{ summary?.orderCount ?? '-' }}</div><div class="card-label">报废单据数</div></el-card></el-col>
    </el-row>
