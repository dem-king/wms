...
import { getOutboundSummary, getOutboundTrend, getOutboundCategoryDistribution, exportReportExcel, exportReportPdf } from '@/api/report'
import { downloadBlob, getExportFilename } from '@/utils/export'
import type { OutboundSummaryVo, OutboundTrendVo, OutboundDistributionVo } from '@/types/report'

const loading = ref(false)
const summary = ref<OutboundSummaryVo | null>(null)
const trendData = ref<OutboundTrendVo | null>(null)
const distributionData = ref<OutboundDistributionVo | null>(null)
...
async function handleQuery() {
  ...
  try {
    ...
    const [summaryRes, trendRes, distRes] = await Promise.all([
      getOutboundSummary(params),
      getOutboundTrend({ ...params, trendType: 'DAILY' }),
      getOutboundCategoryDistribution(params)
    ])
...
function buildTrendChart(data: OutboundTrendVo) {
...
<template>
  <div class="app-container">
    ...
    <el-row :gutter="20" class="summary-cards" v-loading="loading">
      <el-col :span="8"><el-card shadow="hover"><div class="card-value">{{ summary?.totalQuantity ?? '-' }}</div><div class="card-label">出库总数量</div></el-card></el-col>
      <el-col :span="8"><el-card shadow="hover"><div class="card-value">{{ summary?.totalAmount ?? '-' }}</div><div class="card-label">出库总金额</div></el-card></el-col>
      <el-col :span="8"><el-card shadow="hover"><div class="card-value">{{ summary?.orderCount ?? '-' }}</div><div class="card-label">出库单据数</div></el-card></el-col>
    </el-row>
